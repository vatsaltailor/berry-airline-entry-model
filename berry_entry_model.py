"""Berry airline entry model with heterogeneous firms.

Implements maximum likelihood estimation of a static entry game for U.S. airline
markets, following the specification in Berry (1992).
...
# Suppress pandas/bottleneck warning for clean output
...
def calculate_profits(X, airline_sizes, beta):
    """Calculate deterministic profits V_{i,m} = X_m' * beta + airline_size_i."""
    base_profit = X @ beta
    V = base_profit[:, np.newaxis] + airline_sizes[np.newaxis, :]
    return V
...
def calculate_entry_probabilities(V, delta):
    ...
    Uses Berry (1992) sequential entry assumption where firms with higher
    expected profits move first.
    """
...
def estimate_model(data_path='Berry_data.csv', verbose=True):
    """Estimate the Berry airline entry model via maximum likelihood."""
    if verbose:
        print("="*70)
